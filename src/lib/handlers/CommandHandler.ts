...
export default class CommandHandler {
    ...
    public static loadSlashCommandFile(slashCmdFilePath: string): Promise<boolean> {
        return new Promise(async (resolve) => {
            ...
            if (this.slashCommandsCollection.has(slashCmdFileData.buildData.name)) {
                Debug.logWarning(`A slash cmd function with the name "${slashCmdFileData.buildData.name}" has already been loaded (${fileName})`, loggerID)
                resolve(false)
            }

            // Load command function and tags into their respective collections, using the command name as the key
            const slashCommand = (slashCmdFileData.autocomplete) ? new SlashCommandFileWithAutocomplete(slashCmdFileData, fileName) : new SlashCommandFile(slashCmdFileData, fileName) 
     ...
     */
    public static getSlashCommand(interaction: Discord.ChatInputCommandInteraction | Discord.AutocompleteInteraction): SlashCommandFile | undefined {
...
export class SlashCommandFile {
    ...
    protected fileName: string // used as the Debug loggerID for the command
